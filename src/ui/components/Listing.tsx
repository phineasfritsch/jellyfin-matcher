...
export function Bar({
  ...
  return (
    <div className="pane-thin sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-[var(--color-hairline)] px-4 py-3">
      <span className={`text-[15px] font-semibold tracking-[-0.01em] ${ink}`}>{left}</span>
      {right && (
        <span className="tabular shrink-0 text-[13px] font-medium text-muted-fg">{right}</span>
      )}
    </div>
  );
}
...
export function Group({
  ...
  return (
    <section aria-label={ariaLabel} className="px-3 pt-3">
      {title && (
        <h2 className="px-1.5 pb-2 text-[13px] font-medium uppercase tracking-[0.04em] text-muted-fg">
          {title}
        </h2>
      )}
...
function RowBody({
  ...
  return (
    <>
      <span
        className={`flex items-center justify-center py-3.5 text-[11px] font-bold uppercase tracking-[0.05em] ${TONE[tone]}`}
      >
        {label}
      </span>
      <span className="flex flex-col justify-center gap-1 py-3.5 pr-4 text-left">
        <span className="text-[16px] font-semibold leading-snug tracking-[-0.01em]">{title}</span>
        {detail && <span className="text-[13.5px] leading-relaxed text-muted-fg">{detail}</span>}
        {pill && (
          <span
            className={`mt-1.5 self-start rounded-full bg-current/12 px-2.5 py-1 text-[11.5px] font-semibold ${TONE[pillTone ?? tone]}`}
          >
            {pill}
          </span>
        )}
...
export function CostLine({ headline, detail }: { headline: string; detail?: string }) {
  return (
    <p className="mx-3 mt-3 rounded-[var(--radius-card)] border border-destructive/40 bg-destructive/[0.14] px-4 py-3.5 text-[15px] font-semibold leading-snug text-destructive">
      {headline}
      {detail && (
        <span className="mt-1.5 block text-[13.5px] font-normal leading-relaxed text-foreground/85">
          {detail}
        </span>
      )}
    </p>
...
export function BigButton({
  ...
  return (
    <button
      ...
      className={`min-h-[52px] w-full cursor-pointer rounded-[var(--radius-control)] px-4 py-3.5 text-[16px] font-semibold tracking-[-0.01em] transition active:scale-[0.985] disabled:opacity-50 ${skin}`}
    >
      {children}
    </button>
  );
