...
const BEHAVIOUR: Pin[] = [
  ...
  { id: 'B06', why: 'Connecting state is always cleared, even on a failed join — otherwise the UI hangs. Re-pointed (R150): the pin was the literal `.finally(() => setConnecting(false))`, which is a SHAPE. Consolidating the two rejoin paths into one handler kept every guarantee and turned the pin red anyway. The promise -- nothing leaves the phone stuck on a spinner -- is now executed by useroom.render.test.tsx ("the connecting flag is always let go of": after a rejoin that works, one that is refused, and one that never happens), which a grep cannot do. This pin holds the handler that both endings run through', find: 'const rejoinRefused = (err: unknown) => {' },
];
...
const LATESHOW: Pin[] = [
  ...
  { id: 'T84', why: 'A handler refuses a socket that holds no seat. Each of these read socket.data as { userId: string } and trusted it, so a socket that never joined carried undefined into the store', find: "throw new Error(t('server.notInRoom'))" },
  ...
  { id: 'T102', why: 'Whether the household is being remembered is visible where the host already looks. Adjudicated: the property is intact and grew a second half -- R109 added a direct writability probe beside the count, because a deployment whose cache is unwritable reports zero remembered for a reason no host would recognise (R106, R109)', find: '...(await historyHealth()), writable: await cacheWritable()' },
  ...
  { id: 'T105', why: 'A member leaving the LOBBY re-checks whether the round can start. Lobby leavers are deleted outright, so the room can become all-ready by their going -- and startKnockout was reachable only from setReady, so three members with two ready sat on “Everyone is in. Starting.” until the TTL reaped the room (R108)', find: "if (current.status === 'LOBBY') {" },
  { id: 'T106', why: 'The app asks whether it can actually write its cache. The image runs non-root and a bind mount does not inherit the image directory’s ownership, so the documented quickstart made a container that could not write -- and both writers fail open, so ratings re-fetched every build and the household was never remembered, with no symptom a host would recognise (R109)', find: 'export async function cacheWritable' },
  ...
  { id: 'T42', why: 'A failed rejoin ends the session out loud. Pushing main restarts the server and rooms live in memory, so a deploy mid-night left every phone rendering a deck the server had forgotten (R66). Re-pointed (R149/R150): rooms now survive a restart, so the pinned sentence -- "the server restarted" -- became the WRONG CAUSE to hand somebody. A room that is genuinely not found was reaped by the idle TTL or never had its snapshot written. The claim this pin protects is unchanged and still the point: a refused rejoin ends the session out loud rather than leaving a phone on a deck the server has forgotten', find: 'That room has ended. Start a new one.' },
