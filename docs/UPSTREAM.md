...
| U4 | **A public hostname is safe by default.** Today the default auth mode gates nothing: anyone with the URL reads the library. The README warns; upstream would require a safe default instead. Four options are now laid out in [EXPOSURE.md](EXPOSURE.md) with their costs, and the gate's own framing turns out to be wrong: "safe by default costs the four-second guest join" is true only of `MATCHER_AUTH=all`, which nobody proposed. `create` already exists, keeps the guest join entirely, and costs the host one login per twelve hours (R184). | ❌ a decision, now with options |
| U5 | **Deployment parity verified.** At least one run proving what is deployed matches this repository. Never once done. The TOOL exists and always has — `npm run prod:read` reads `/healthz`, compares the deployed `version` against this checkout and says whether they agree. What is missing is one environment variable, `MATCHER_URL`, pointing at the running server; there has been one behind a tunnel with auto-deploy against it for some time now (R183). This gate is a command the maintainer has not run, not a thing that cannot be done. | ❌ one command, never run |
| U6 | **Used by a household that is not the maintainer, for ten evenings.** With what broke written down — the writing-down IS the gate, and ten evenings with no record produces "it was fine", which nobody outside this repository has reason to believe. [FIELD-LOG.md](FIELD-LOG.md) is where they go and is currently empty, which is the honest state (R185). | ❌ never attempted |
| U7 | **Accessibility conformance stated and measured.** A named target (WCAG 2.2 AA), an audit against it, and the failures listed. | ⚠ partly — audited in [ACCESSIBILITY.md](ACCESSIBILITY.md) against WCAG 2.2 AA. **Nothing is UNVERIFIED any more**: 1.4.12 measured in real Chrome at three viewports (R186), 2.2.1 argued with the Essential exception named correctly (R188), and 1.4.3's definitional pairs gated so they run every time (R187). What remains is three media failures stated and closed — the trailer's captions and audio description, which this app neither supplies nor suppresses (F7) — one **PARTIAL** at 1.4.11 where the slider track is still under 3:1, and one thirty-second manual check nobody has run (R5: tab into a playing trailer, count your way out). |
| U8 | **Internationalisation.** Extraction is done: every sentence the UI says lives in `src/ui/strings.ts`, reached through `t()` or `Sentence`, with the reasoning a translator needs stored beside the load-bearing ones and asserted as data (R145). Three guards keep it that way — no component holds a second copy, no screen writes prose into its markup, and the promises that live in wording go red if a translation undoes them. What is NOT done is selection: there is one catalogue, `en`, and nothing chooses another. The server's own refusals are in it now (R176): 19 sentences from `server/handlers.ts`, which R163 had made reach a phone's banner. Upstream failures stay technical on purpose — "Jellyfin request failed: 502" is a line for the person who can act on it (R54). The server's room-facing copy is catalogued too (R196): the two shutdown notices and the login refusals, which reach a phone exactly as handlers.ts's do. That claim was wrong when it first said `diagnose.ts` was all that remained -- an adversarial review found five other server files writing English, and two of them were sending it to rooms. What DOES remain on the server on purpose is `diagnose.ts` (R177: three of its fix sentences name environment variables, and B03 refuses a credential named in code that ships to guests) and the log and operator lines, which no household reads. Jellyfin ships in dozens of languages. | 🟡 half — extracted, not selectable |
