...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 543 cases in 33 files |
| G3 | pinned claims still pinned | 189 |
...
`src/ui/__tests__/pins.test.ts` holds 189 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 543 cases, 33 files, 189 pins.
