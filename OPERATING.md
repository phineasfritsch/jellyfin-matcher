...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 437 cases in 28 files |
| G3 | pinned claims still pinned | 158 |
...
`src/ui/__tests__/pins.test.ts` holds 158 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 437 cases, 28 files, 158 pins.
