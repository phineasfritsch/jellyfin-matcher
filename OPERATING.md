...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 136 cases in 10 files |
| G3 | pinned claims still pinned | 67 |
...
`src/ui/__tests__/pins.test.ts` holds 67 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 136 cases, 10 files, 67 pins.
