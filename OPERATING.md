...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 425 cases in 28 files |
| G3 | pinned claims still pinned | 153 |
...
`src/ui/__tests__/pins.test.ts` holds 153 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 425 cases, 28 files, 153 pins.
