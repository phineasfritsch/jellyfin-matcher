...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 447 cases in 28 files |
| G3 | pinned claims still pinned | 164 |
...
`src/ui/__tests__/pins.test.ts` holds 164 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 447 cases, 28 files, 164 pins.
