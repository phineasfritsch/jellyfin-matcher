...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 501 cases in 29 files |
| G3 | pinned claims still pinned | 180 |
...
`src/ui/__tests__/pins.test.ts` holds 180 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 501 cases, 29 files, 180 pins.
