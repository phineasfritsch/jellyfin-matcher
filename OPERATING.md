...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 462 cases in 29 files |
| G3 | pinned claims still pinned | 167 |
...
`src/ui/__tests__/pins.test.ts` holds 167 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 462 cases, 29 files, 167 pins.
