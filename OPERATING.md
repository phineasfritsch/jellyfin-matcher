...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 463 cases in 29 files |
| G3 | pinned claims still pinned | 168 |
...
`src/ui/__tests__/pins.test.ts` holds 168 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 463 cases, 29 files, 168 pins.
