...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 377 cases in 25 files |
| G3 | pinned claims still pinned | 143 |
...
`src/ui/__tests__/pins.test.ts` holds 143 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 377 cases, 25 files, 143 pins.
