...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 375 cases in 25 files |
| G3 | pinned claims still pinned | 141 |
...
`src/ui/__tests__/pins.test.ts` holds 141 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 375 cases, 25 files, 141 pins.
