...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 443 cases in 28 files |
| G3 | pinned claims still pinned | 160 |
...
`src/ui/__tests__/pins.test.ts` holds 160 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 443 cases, 28 files, 160 pins.
