...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 273 cases in 18 files |
| G3 | pinned claims still pinned | 117 |
...
`src/ui/__tests__/pins.test.ts` holds 117 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 273 cases, 18 files, 117 pins.
