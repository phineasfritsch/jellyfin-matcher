...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 280 cases in 18 files |
| G3 | pinned claims still pinned | 120 |
...
`src/ui/__tests__/pins.test.ts` holds 120 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 280 cases, 18 files, 120 pins.
