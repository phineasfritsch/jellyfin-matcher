...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 490 cases in 29 files |
| G3 | pinned claims still pinned | 172 |
...
`src/ui/__tests__/pins.test.ts` holds 172 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 490 cases, 29 files, 172 pins.
