...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 334 cases in 20 files |
| G3 | pinned claims still pinned | 129 |
...
`src/ui/__tests__/pins.test.ts` holds 129 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 334 cases, 20 files, 129 pins.
