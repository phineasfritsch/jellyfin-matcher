...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 446 cases in 28 files |
| G3 | pinned claims still pinned | 163 |
...
`src/ui/__tests__/pins.test.ts` holds 163 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 446 cases, 28 files, 163 pins.
