...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 520 cases in 31 files |
| G3 | pinned claims still pinned | 181 |
...
`src/ui/__tests__/pins.test.ts` holds 181 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 520 cases, 31 files, 181 pins.
