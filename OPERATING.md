...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 445 cases in 28 files |
| G3 | pinned claims still pinned | 162 |
...
`src/ui/__tests__/pins.test.ts` holds 162 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 445 cases, 28 files, 162 pins.
