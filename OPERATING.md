...
|---|---|---|
| G1 | `tsc --noEmit` | clean |
| G2 | `vitest run`, and the **counts** | 365 cases in 24 files |
| G3 | pinned claims still pinned | 138 |
...
`src/ui/__tests__/pins.test.ts` holds 138 claims that nothing else asserts:
...
2. **The gate, numbered, with today's numbers**: 365 cases, 24 files, 138 pins.
